...
- bsremote: Add a git remote for matching BS repository.
...
echo "z$target" | grep -q "-"
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the BS model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
...
function bsremote()
{
    ...
    git remote add cm git@github.com:CyanogenModRoms/$PFX$PROJECT
    echo "Remote 'cm' created"
    fi
}
